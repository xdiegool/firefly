/**
 * @file
 * @brief The public API of the transport Ethernet LWIP with specific structures and
 * functions.
 */
#ifdef _GNU_SOURCE
...
void firefly_transport_eth_lwip_read(struct firefly_transport_llp *llp)
{
	...
	int res;
	struct transport_llp_eth_lwip *llp_eth = (struct transport_llp_eth_lwip *) llp->llp_platspec;
	fd_set fs;
	FD_ZERO(&fs);
