	...
	struct firefly_event_queue *event_queue)
{
	...
	llp_tcp->local_tcp_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_TCP);
	...
	llp_tcp->resend_queue = firefly_resend_queue_new();
...
void firefly_transport_llp_tcp_posix_free(struct firefly_transport_llp *llp)
{
	...
	ret = llp_tcp->event_queue->offer_event_cb(llp_tcp->event_queue,
			FIREFLY_PRIORITY_LOW,
			firefly_transport_llp_tcp_posix_free_event,
			llp, 0, NULL);
...
static void check_llp_free(struct firefly_transport_llp *llp)
{
	...
	if (llp->state == FIREFLY_LLP_CLOSING && llp->conn_list == NULL) {
		...
		close(llp_tcp->local_tcp_socket);
		free(llp_tcp->local_addr);
		firefly_resend_queue_free(llp_tcp->resend_queue);
		free(llp_tcp);
		free(llp);
	}
}

int firefly_transport_llp_tcp_posix_free_event(void *event_arg)
{
	struct firefly_transport_llp *llp;
	llp = event_arg;
		...
		unsigned int timeout)
{
	...
	struct firefly_transport_connection_tcp_posix *tcup;
	struct transport_llp_tcp_posix *llp_tcp = llp->llp_platspec;
	int res;
	tc = malloc(sizeof(*tc));
	...
	tcup->socket = llp_tcp->local_tcp_socket;
	tcup->llp = llp;
	...
	tc->ack = firefly_transport_tcp_posix_ack;
	return tc;
}

void firefly_transport_tcp_posix_ack(unsigned char pkt_id,
		struct firefly_connection *conn)
{
	struct firefly_transport_connection_tcp_posix *conn_tcp;
	struct transport_llp_tcp_posix *llpup;

	conn_tcp = conn->transport->context;
	llpup = conn_tcp->llp->llp_platspec;
		...
		struct firefly_connection *conn, bool important, unsigned char *id)
{
	...
	conn_tcp = conn->transport->context;
	res = sendto(conn_tcp->socket, data, data_size, 0,
		     (struct sockaddr *) conn_tcp->remote_addr,
		     sizeof(*conn_tcp->remote_addr));
	if (res == -1) {
	...
	if (important) {
		...
		struct transport_llp_tcp_posix *llp_ps;
		...
		llp_ps = conn_tcp->llp->llp_platspec;
		*id = firefly_resend_add(llp_ps->resend_queue,
				new_data, data_size, conn_tcp->timeout,
				FIREFLY_TRANSPORT_TCP_POSIX_DEFAULT_RETRIES, conn);
	}
}

void *firefly_transport_tcp_posix_read_run(void *args)
{
	struct firefly_transport_llp *llp;
...
int firefly_transport_tcp_posix_run(struct firefly_transport_llp *llp)
{
	...
	if (!largs) {
		pthread_cancel(llp_tcp->read_thread);
		return -1;
	}
	largs->rq = llp_tcp->resend_queue;
	largs->on_no_ack = resend_on_no_ack;
	res = pthread_create(&llp_tcp->resend_thread, NULL,
...
int firefly_transport_tcp_posix_stop(struct firefly_transport_llp *llp)
{
	...
	llp_tcp = llp->llp_platspec;
	pthread_cancel(llp_tcp->read_thread);
	pthread_cancel(llp_tcp->resend_thread);
	pthread_join(llp_tcp->resend_thread, NULL);
	pthread_join(llp_tcp->read_thread, NULL);
	return 0;
}
...
void firefly_transport_tcp_posix_read(struct firefly_transport_llp *llp)
{
	...
	do {
		FD_ZERO(&fs);
		FD_SET(llp_tcp->local_tcp_socket, &fs);
		res = select(llp_tcp->local_tcp_socket + 1, &fs, NULL, NULL, NULL);
	...
	res = ioctl(llp_tcp->local_tcp_socket, FIONREAD, &pkg_len);
	if (res == -1) {
	...
	res = recvfrom(llp_tcp->local_tcp_socket, ev_arg->data, pkg_len, 0,
	...
	llp_tcp->event_queue->offer_event_cb(llp_tcp->event_queue,
			FIREFLY_PRIORITY_HIGH,
			firefly_transport_tcp_posix_read_event,
			ev_arg, 0, NULL);
}

int firefly_transport_tcp_posix_read_event(void *event_arg)
{
	struct firefly_event_llp_read_tcp_posix *ev_arg;
	struct transport_llp_tcp_posix *llp_tcp;
	struct firefly_connection *conn;
	...
	if (conn == NULL) {
		...
		int64_t ev_id = 0;
		if (llp_tcp->on_conn_recv != NULL &&
				(ev_id = llp_tcp->on_conn_recv(ev_arg->llp, ip_addr,
					sockaddr_in_port(&ev_arg->addr))) > 0) {
			return llp_tcp->event_queue->offer_event_cb(llp_tcp->event_queue,
					FIREFLY_PRIORITY_HIGH,
					firefly_transport_tcp_posix_read_event,
					ev_arg, 1, &ev_id);
		}
...
{
	return sockaddr_in_eq(((struct firefly_transport_connection_tcp_posix *)
